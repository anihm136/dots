return {
	...
	config = function()
		...
		map(
			"n",
			"<leader>ff",
			function() file_search(vim.fn.expand("%:p:h")) end,
			"Files in current file's directory"
		)
		map(
			"n",
			"<leader>fp",
		...
		map(
			"n",
			"<leader>sd",
			function() live_grep(vim.fn.expand("%:p:h")) end,
			"Live grep in current file's directory"
		)
